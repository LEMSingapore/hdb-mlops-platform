...
8. **(D) Model on the box:** train once on the box after first deploy (`docker compose run` the training stage). This roots the MLflow registry's artifact paths to the box's own filesystem rather than this Mac's, but it does **not** remove the absolute-path issue found in Phase 2 Session B — the `scripts/migrate_artifact_paths_to_proxy.py` step is still a prerequisite before `docker compose up`, because the containerised tracking server serves artifacts over `mlflow-artifacts:` proxy URIs, not filesystem paths. This resurfaced during the box deploy. Requires the training data (SQLite DB ~162MB, or rebuilt from data.gov.sg CSVs) present on the box as a one-time step.
